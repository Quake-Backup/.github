#!/usr/bin/bash
# scripts/update-deletions-issue.sh
# Crea o edita el issue de tracking con el reporte de deleciones.
#
# Uso: ./scripts/update-deletions-issue.sh <report-file> <target-repo>
...
report_file="$1"
target_repo="$2"
issue_title="[bot] Quake-Backup fork deletions"
...
if ! gh label list --repo "$target_repo" --json name \
    ...
    gh label create "$label" --repo "$target_repo" \
        --color "d73a4a" \
        --description "Repos deleted from the org" \
