#!/usr/bin/bash
# scripts/update-additions-issue.sh
# Crea o edita el issue de tracking con el reporte de adiciones.
#
# Uso: ./scripts/update-additions-issue.sh <report-file> <target-repo>
...
report_file="$1"
target_repo="$2"
issue_title="[bot] Quake-Backup fork additions"
label="addition-report"
...
if ! gh label list --repo "$target_repo" --json name \
    ...
    gh label create "$label" --repo "$target_repo" \
        --color "0e8a16" \
        --description "Repos added to the org" \
